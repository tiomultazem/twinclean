...
class DuplicateFinderApp:
    ...
    def on_hover(self, event):
        ...
        # Cegah error kalau baris hover lama sudah tidak ada
        if getattr(self, "tree_hover_row", None):
            if self.tree_hover_row not in self.tree.get_children():
                self.tree_hover_row = None

        if region == "cell" and row_id:
            if self.tree_hover_row != row_id:
                if self.tree_hover_row:
                    # Pastikan item lama masih ada sebelum ubah tag-nya
                    if self.tree_hover_row in self.tree.get_children():
                        self.tree.item(self.tree_hover_row, tags=())
                self.tree_hover_row = row_id
                # Pastikan item baru juga ada sebelum diberi tag
